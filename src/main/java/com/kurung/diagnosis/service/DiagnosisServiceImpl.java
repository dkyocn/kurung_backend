...
public class DiagnosisServiceImpl implements DiagnosisService {
  ...
  public void createAnswers(List<AnswerDTO> answerList) {
    ...
    // 사용자 UUID 추출
    String userUuid = answerList.get(0).getUser().getUserUuid();

    // 기존 응답 유무 확인
    List<HealthAnswerEntity> answerEntityList = answerRepository.getByUserUuid(userUuid);
...
//      }
      ...
      HealthQuestionEntity questionEntity = questionRepository.getQuestionById(dto.getQuestionId());
      ...
      // ✅ 여기서 OptionId로 조회
      HealthOptionEntity optionEntity = optionRepository.getByOneOptionId(dto.getOption().getOptionId());
      if (dto.getOption() != null && dto.getOption().getOptionId() > 0) {
      ...
      HealthAnswerEntity answerEntity = HealthAnswerEntity.builder()
          .user(UserEntity.builder().userUuid(dto.getUser().getUserUuid()).build())
